...
class FrontEnd(Gtk.VBox):
	...
	def __init__(self, *args, **kwargs):
		...
		self.bill_number_fromdb = int(db.get_and_update_bill_number_on_init()) # Get Bill number from database
		...
		date_label.set_markup("<b>Date: " + current_date.strftime("%d/%m/%Y")+"</b>")
		...
		# Bottom Container
		bottom_box = Gtk.Box(False, False, 0)
	...
	def add_item_to_bill(self):
		...
		food_item = self.item_list_from_database.get(int(item_code))
		if food_item == None:
			InformationDialog("Invalid Item Code", "Enter Valid Item Code")
		else:
			self.total_amount += food_item.get("cost")
			self.total_Entry.set_markup("<span font_desc='Arial 20'>{}</span>".format(str(self.total_amount)))
			if item_code not in self.item_list_for_bill:
				temporary_item_for_bill = [food_item.get("item_name"), food_item.get("cost"), 1, food_item.get("cost")]
				self.item_list_for_bill.setdefault(int(item_code), temporary_item_for_bill)
			else:
	...
	def update_bill(self):
		...
		for item_code in self.item_list_for_bill:
			self.bill_list_store.append(list(self.item_list_for_bill.get(int(item_code))))
		for i, column_title in enumerate(["Food Item", "Price", "Quantity", "Total"]):
			renderer = Gtk.CellRendererText()
			column = Gtk.TreeViewColumn(column_title, renderer, text = i)
			column.set_min_width(150)
	...
	def final_bill(self):
		...
		if response == 1:
			bill = Bill()
			bill.generate_bill(self.bill_number_fromdb, self.item_list_for_bill, self.total_amount)
			self.clear_bill()
		elif response == 2:
			self.clear_bill()

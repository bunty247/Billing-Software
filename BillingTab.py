...
class BillingTab(Gtk.VBox):
	...
	def __init__(self, *args, **kwargs):
		...
		self.customer_name_entry = Gtk.Label()
	...
	def _get_customer(self):
		...
		customer = db.get_customer_details(mobile_number)
		if customer == None:
			self._add_new_customer()
		else:
			self.customer_name_entry.set_text(customer.customer_name)
			self.reward_count_label.set_text(str(customer.reward_points))

	def _add_new_customer(self):
		dialog_box = Gtk.Dialog("New Customer", None, Gtk.DialogFlags.MODAL, (Gtk.STOCK_OK, 1))
		dialog_box.set_size_request(150, 100)
		box = dialog_box.get_content_area()
		box.add(Gtk.Label("Add New Customer"))
		hbox = Gtk.HBox()
		hbox.pack_start(Gtk.Label("Customer Name: "), False, 0, 0)
		name_entry = Gtk.Entry()
		hbox.pack_start(name_entry, False, 0, 0)
		box.add(hbox)
		dialog_box.show_all()
		response = dialog_box.run()
		if response == 1:
			db = DatabaseOperations()
			db.add_new_customer(int(self.mobile_number_entry.get_text()), name_entry.get_text())
			self.customer_name_entry.set_text(name_entry.get_text())
			dialog_box.destroy()
		dialog_box.destroy()
	...
	def _final_bill(self):
		...
		if response == 1:
			...
			db.update_reward_points(
				self.mobile_number_entry.get_text(), self.total_amount)
	...
	def _clear_bill(self, *args):
		...
		self.total_amount = 0
		self.customer_name_entry.set_text("")
		self.reward_count_label.set_text("0")
		self.mobile_number_entry.set_text("")
		self.total_Entry.set_markup(
			"<span font_desc='Arial 20'>{}</span>".format(str(0)))
